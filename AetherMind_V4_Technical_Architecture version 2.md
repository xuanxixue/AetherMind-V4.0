...
> Version: 4.0.0\
> Base Model: Xirang - Dual-Weight Evolutionary Cognitive System [AetherMind V4.0 — A Self-Evolving Language-Physics Model That "Grows Its Brain"]
...
| **Version** | 4.0.0 |
| **Architecture Base** | AetherMind 3.6.1 + PGTA Pheromone-Guided Thermodynamic Attention + Evolvable Dual-Weight System (W/τ) + LTP Consolidation |
...
When using, modifying, distributing, or creating derivative works, **you must attribute the original author "Xuanxi Xue (Zhang Yue)" and indicate the source link**. Commercial use is permitted.

### 📅 Open Source Announcement

> 🚀 **Model code is expected to be officially open-sourced between September 5 – September 25, 2026** (within 10–30 days from now). Stay tuned.
...
- **PGTA Pheromone-Guided Thermodynamic Attention**: Attention weights are computed as $A = \text{softmax}(-E_{\text{eff}}/T)$, where the energy term $E_{\text{eff}}$ fuses pheromone traces τ, thermodynamic entropy terms, and learnable bias; temperature T is estimated in real-time by the meta-cognitive module TriPercept, achieving "answer decisively when confident, explore actively when uncertain."
- **Dual-Weight System**: Fast weights W are trained via standard BP (Phase A), slow weights τ evolve online through physical rules (Phase B/C); the two are fused with gating coefficient λ, achieving synergy between "innate ability" and "acquired experience."
- **GMM Dual-Domain Knowledge Atoms**: 1024 knowledge atoms span two domains — Language (L) and Physics (P) — each atom modeled by 3 Gaussian components as a sparse distributed representation of knowledge.
- **Langevin Oscillator**: A Kuramoto phase synchronization model generates physical phase θ for each token, projected via phys_proj into cognitive injection vector $Z_{\text{phys}}$, introducing continuous physical signals into discrete token processing.
- **GLUBlockV4 Decoder**: Integrates TSR (Token State Router, gated memory write) and PSR (Phase State Router, phase-gated state routing), achieving dual state management at both token-level and phase-level.
- **Meta-Cognitive Gating**: TriPercept three-MLP median voting produces cognitive uncertainty $u_{\text{cog}}$, driving temperature $T = T_0(1 + \kappa \cdot u_{\text{cog}})$ dynamic adjustment; SafetyFilter intercepts high-risk outputs.

Training adopts a **three-phase curriculum**: Phase A (pure BP pre-training, τ=1 uniform) establishes foundational language ability; Phase B (mixed training, gradual λ introduction) guides pheromone deposition with dLoss as reward; Phase C (evolution + consolidation) switches to dF free energy reward, temperature cools to 0.2, and LTP consolidation is triggered to enter the self-organization phase. The trainer includes checkpoint resumption (precise to batch, including evolver state), NaN/Inf protection, bf16 mixed precision, and gradient accumulation, optimized for consumer GPUs like the RTX 3050 4GB, capable of completing 35,000 training steps in approximately 6–8 hours.

**In one sentence**: V4 attempts to answer "can a model continue to grow its brain after deployment" — not through bolted-on RAG or retraining, but by making the weights themselves plastic.

***

## Chapter 0. Document Conventions
...
## Chapter 1. Design Motivation and Core Proposition
...
input_ids (B,S) ──────────────────► │  TOKEN EMBEDDING                                          │
            ...
            │ Logic Domain (L)     │          │  ┌────────────────┐  │           │  routed to physics)  │
            │ Poetic Domain (P)    │          │  │ EpisodicMem L  │  │           │                      │
            │                      │          │  │ EpisodicMem P  │  │           │  mu_L/P: (Na,Da)     │
            │ r0 = ||W_r(mu)||     │          │  │ 4096 slots/dom │  │           │  ω: (Na,) eigenfreq  │
            ...
            │ META COGNITIVE GATE                                            │
            │                                                                │
            │ TriPercept(3×MLP voting):                                         │
            │   cat(z_L'.mean(S), z_P'.mean(S), task_emb) → 3×MLP → softmax  │
            │   α_L = median(α₁,α₂,α₃)                                       │
            │   u_cog = Var(α₁,α₂,α₃) / 0.25   (cognitive uncertainty)       │
            │                                                                │
            │ Physics fusion: z_L = (1-λ)z_IB_L' + λ·Z_phys_L               │
            ...
            └────────────────────────────┬────────────────────────────────────┘
                                    ...
                                    │  │    (independent PGTA per layer,  │    │
                                    │  │     temperature set by meta_T)  │    │
                                    │  │    A = softmax(-E_eff/T)        │    │
                   ...
                   │ L_LM = CE(shift)    │                     │ (non-gradient, called      │
                   │ L_IB = KL_L + KL_P  │                     │  post-step)               │
                   │ L_D = BCE(D,0.5)    │                     │                           │
                   │ L_aff = λ·std(z_P)  │                     │ Phase A: no-op            │
                   │ L_H = λ·ReLU(H-H*)  │                     │ Phase B:                  │
                   │ L_mod = module_loss │                     │   r = -loss               │
                   │ L_anc = anchor_loss │                     │   τ ← (1-ρ)τ + η·r·Ā     │
                   │ L_phys = λ·F²       │                     │ Phase C:                  │
                   │                     │                     │   r = -dF/(|dF|+1)        │
                   │ L_total = Σ λ_i L_i │                     │   τ ← (1-ρ)τ + η·r·Ā     │
                   │         + L_LM      │                     │   every 100 steps:         │
                   │                     │                     │     consolidate():         │
                   │                     │                     │     if τ>θ: C+=λ(τ-θ)     │
                   │                     │                     │            τ*=(1-γ)        │
                   └─────────────────────┘                     └──────────────────────────┘
...
## Chapter 3. PGTA Pheromone-Guided Thermodynamic Attention (V4 Core Innovation)
...
- $C$: Consolidated weight bias, see Section 3.7
...
## Chapter 4. Evolvable Dual-Weight System
...
## Chapter 5. GMM Dual-Domain Knowledge Atoms (Inherited from 3.6)
...
## Chapter 6. Langevin Oscillator (Inherited from 3.6)
...
for step in range(K):
    ...
    if requires_grad:
        r_grad = -grad(F, r, create_graph=True)[0] # Analytic gradient
    else:
        r_grad = -2 * (r - r0)                     # Hooke's force fallback
    xi_r = randn(N) * sqrt(2 * T_enc * dt)
    r = r + dt * r_grad + xi_r
    r = clamp(r, min=1e-3)                         # Prevent collapse
```
...
## Chapter 7. Dual Memory System (Inherited from 3.6)
...
## Chapter 8. Meta-Cognitive Gating (Inherited from 3.6)
...
## Chapter 9. V4 Encoder (EncoderV4)
...
## Chapter 10. V4 Decoder (GLUBlockV4)
...
## Chapter 11. AetherMind4 Main Model
...
## Chapter 13. Complete Hyperparameter Reference
...
| d_ff | 2,048 | 1,536 | Feed-forward network dimension (4×D) |
| n_layers | 8 | 6 | Number of decoder layers |
| n_heads | 8 | 6 | Number of attention heads |
| max_seq_len | 1,024 | 512 | Maximum sequence length |
| dropout | 0.1 | 0.1 | Dropout rate |
| n_atoms | 1,024 | 1,024 | Number of GMM atoms |
...
| max_consolidations | 4,096 | Consolidation budget upper limit |
...
| warmup_steps | 1,000 | LR warmup steps |
...
| eval_interval | 2,000 | Evaluation interval |
| save_interval | 5,000 | Save interval |

***

## Chapter 14. VRAM and Compute Budget
...
## Chapter 15. Complete Loss Function Derivation
...
## Chapter 16. Comparison with V3.6 / Mainstream Models
...
## Chapter 17. Known Limitations and Future Directions
...
## Chapter 18. File Structure and Code Index
...
## Chapter 19. Verification Status
...
## Chapter 20. 2026-08-27 Practical Adjustment Record

> **Note**: This chapter records the adjustments made after the first practical training run of V4.0 (d=384, L=6, h=6, seq=512, RTX 3050 4GB). Original chapter content is neither deleted nor modified; all changes are documented here. Each adjustment includes: problem symptoms, root cause analysis, modification location, specific changes, and before/after comparison.

### 20.1 Tokenizer Replacement: Custom BPE → Qwen2.5-0.5B

**Problem**: During inference, all Chinese output was `<unk>`, and English output was fragmented. The custom BPE tokenizer could not correctly encode Chinese text.

**Root Cause**: The custom BPE tokenizer used by the original V4 had a vocabulary of only 50,000 tokens, with insufficient Chinese coverage in the training data, causing many Chinese characters to be mapped to the unk token.

**Modification Location**:

- `configs/aethermind4_config.py`: `vocab_size`, `tokenizer_path`, `pad/eos/unk_token_id`
- `src/data/dataset.py`: `build_tokenizer()` function completely rewritten

**Specific Changes**:

| Parameter | Before | After |
|---|---|---|
| `vocab_size` | 50,000 | 151,936 (aligned to 128 multiple → 151,680) |
| `tokenizer_path` | None (built-in BPE) | `models_local/Qwen/Qwen2___5-0___5B` |
| `pad_token_id` | 0 | 151,643 |
| `eos_token_id` | 1 | 151,643 |
| `unk_token_id` | 2 | 151,643 |

The `build_tokenizer()` in `dataset.py` was changed to use `transformers.AutoTokenizer.from_pretrained()` to load the Qwen2.5-0.5B tokenizer, wrapped with a unified interface (`encode()`/`decode()`/`vocab_size` property). All `self.tokenizer(...)` calls in the data loading pipeline remain unchanged, with compatibility maintained through the wrapper layer.

**Impact**: The Embedding layer and LM Head weight matrices changed from `(50,000, 384)` to `(151,680, 384)`, increasing parameters from approximately 19M to 58M (total 101M), but the model dimensions d=384/L=6/h=6/seq=512 remained unchanged.

***

### 20.2 Bug1 Fix: Auxiliary Losses Not Disabled During Evaluation

**Problem**: Training logs showed eval loss was abnormally high, and pheromones were still evolving during eval (tau values changing), making evaluation results incomparable.

**Root Cause**: The `evaluate()` method in `train_v4.py` simply called `model.eval()`, but did not switch the config's phase parameter. The model in eval mode still loaded Phase B/C auxiliary loss weights (lambda_phys, lambda_PSR, etc.), and pheromone deposition/evaporation was still active, causing the evaluation loss to include non-LM terms such as physics loss.

**Modification Location**: `src/training/train_v4.py` → `evaluate()` method

**Specific Fix**: Before evaluation, save all current lambda weights, langevin_K, and pheromone parameters; call `cfg.set_phase_A()` to switch to pure LM mode (all auxiliary lambda=0, K=0, deposit=0, rho=0); after evaluation, restore original parameters:
...
cfg.set_phase_A()  # Switch to pure LM mode
...
### 20.3 Bug2 Fix: Phase C Auxiliary Loss Weights Too Large

**Problem**: After entering Phase C, loss increased instead of decreasing, the training curve showed obvious jumps, and auxiliary losses dominated the total loss.

**Root Cause**: In the original Phase C configuration, the sum of all auxiliary loss weights was approximately 3.49, while the main LM loss weight was only 1.0. The auxiliary terms (physics loss, PSR loss, IB loss, etc.) had large gradient magnitudes, causing the model to prioritize optimizing auxiliary objectives over language modeling.

**Modification Location**: `configs/aethermind4_config.py` → `set_phase_B()`, `set_phase_C()`

**Specific Fix**: All auxiliary lambda weights were halved and multiplied by a progress ramp-up coefficient:

| Parameter | Phase B Before | Phase B After | Phase C Before | Phase C After |
|---|---|---|---|---|
...
| **Total auxiliary weight** | **~2.0** | **~0.6–1.2** | **~3.49** | **~1.3** |

***

### 20.4 Bug3 Fix: Learnable Temperature Being Force-Overridden

**Problem**: In training logs, `T` was always a fixed value (e.g., 1.0 or 0.2), the learnable temperature parameter `log_temp` had no gradient changes, and the meta-cognitive gating temperature regulation was ineffective.

**Root Cause**: In the forward method of `aethermind4.py`, `self.set_temperature(T0)` was called to directly overwrite `self.log_temp.data`, hard-writing the meta-cognitive module's computed temperature into the learnable parameter, causing `log_temp`'s gradients to be overwritten and preventing BP learning.

**Modification Location**: `src/model/aethermind4.py` → forward method, PGTA attention calls

**Specific Fix**:

1. Removed all `self.set_temperature(...)` calls
2. Added a `temperature_override` parameter to PGTA attention's forward; during forward computation, the temperature is passed via this parameter without modifying `self.log_temp` itself
3. During training, `temperature_override=None`, and attention internally uses the learnable `self.log_temp`; during inference/generation, the meta-cognitive temperature `T_tensor` is passed in
4. Decoder layers work similarly — temperature is passed via parameter to each layer, no longer overwriting module attributes

```python
# Before (incorrect):
self.set_temperature(T0)  # Directly overwrites log_temp.data
attn_out = self.attn(x)

# After (correct):
attn_out, _ = self.attn(norm1(x), temperature_override=T)
# attention internal: T_actual = override if override is not None else torch.exp(log_temp)
```

***

### 20.5 Bug4 Fix: Pheromone Evolution Not Activated

**Problem**: In training logs, `tau=1.0` (initial value) remained unchanged, `cons=0` (consolidation count was 0), and the pheromone network had no deposition at all.

**Root Cause**: Four factors combined to prevent effective pheromone deposition:

1. Evaporation rate `rho=0.05` was too high — 5% evaporation per step, and the deposition amount `deposit=0.05` was insufficient to compensate
2. Consolidation threshold `consolidate_threshold=2.0` was too high — tau starting from 1.0 could never reach it
3. The reward signal was not scaled — dLoss/dF magnitude was very small (~0.01), so deposition per step ≈ reward × deposit ≈ 0.0005
4. Phase B's deposit initial value was 0, requiring progress ramp-up, but progress grew slowly

**Modification Location**: `configs/aethermind4_config.py`

**Specific Fix**:

| Parameter | Before | After | Description |
|---|---|---|---|
| `pheromone_rho` | 0.05 | 0.02 | Reduced evaporation rate, easier deposition retention |
| `pheromone_deposit` (Phase C) | 0.05 | 0.12 | Increased deposition strength |
| `pheromone_deposit` (Phase B) | 0.05×prog | 0.08×prog | Increased gradual deposition |
| `consolidate_threshold` | 2.0 | 1.2 | Lowered consolidation threshold, tau triggers at 1.2 |
| `reward_scale` | 1.0 | 5.0 | Amplified dLoss/dF reward signal |

Post-fix training log verification: at step 12000, `tau=73.1`, pheromones are effectively deposited and continuously growing.

***

### 20.6 VRAM Optimization: Gradient Checkpointing

**Problem**: With the d=384 config on RTX 3050 4GB, forward+backward peak VRAM exceeded 4GB, causing CUDA OOM.

**Modification Location**:

- `configs/aethermind4_config.py`: Added `gradient_checkpointing: bool = True`
- `src/model/aethermind4.py`: `EncoderV4.forward()` and `AetherMind4.forward()` decoder layer loop

**Specific Fix**: Using `torch.utils.checkpoint.checkpoint()` to wrap each encoder and decoder layer:
...
# Decoder layers similarly
```

**Effect**: Activation VRAM saved approximately 50% (~1.0GB), at the cost of approximately 20% slower training speed (forward needs recomputation). Not enabled during inference (`self.training=False`), so no speed impact on inference.

***

### 20.7 VRAM Optimization: Skip Softmax and Pointer Copy During Training

**Problem**: Even with gradient checkpointing enabled, the LM Head's softmax and pointer copy still produced large tensors causing OOM.

**Root Cause**:

1. `out_prob = softmax(logits/T)` produces a float tensor of shape `(1, 512, 151,680)`, approximately 310MB
2. Pointer copy's `F.one_hot(input_ids, num_classes=151,680)` produces a float tensor of shape `(1, 512, 151,680)`, approximately 310MB
3. These two tensors are completely unnecessary during training — training uses `F.cross_entropy(logits, labels)` to compute loss directly, without needing prob

**Modification Location**: `src/model/aethermind4.py` → LM Head section

**Specific Fix**:

```python
# During training (labels is not None), skip softmax/prob computation
out_prob = None
if labels is None:
    # Only compute prob during inference/generation
    out_prob = safe_softmax(logits.float(), T=T_safe).to(h.dtype)
    # Pointer copy only enabled for small vocabularies
    use_pointer = (cfg.vocab_size <= 65536) and ...
    if use_pointer:
        ...
```

The pointer copy mechanism (copying the input token's probability distribution) requires creating a `(B, S, V)` one-hot tensor under large vocabulary, approximately 310MB when V=151,680. Added vocabulary size check: pointer copy is only enabled when `vocab_size <= 65536`. Under large vocabulary, it is directly disabled. This mechanism provides minimal benefit for large vocabularies (copying 512 out of 151,680 already-occurring tokens), but the VRAM cost is enormous.

**Effect**: Saves approximately 600MB of VRAM during training (softmax 310MB + one-hot 310MB). Full functionality is retained during inference.

***

### 20.8 VRAM Optimization: Release Tensors Before Backward

**Modification Location**: `src/training/train_v4.py` → `_train_step()`

**Specific Fix**: After forward completes and before backward, immediately extract all scalar statistics to CPU, then delete all large tensors in the out dictionary:
...
# Immediately release forward output large tensors
del out, input_ids, labels

# backward (at this point out/logits and other large tensors are released)
self.scaler.scale(loss).backward()
del loss
```

Inside the LM Head, `del shift_logits, shift_labels` is also called immediately after computing cross_entropy.

Additionally, `gc.collect()` + `torch.cuda.empty_cache()` is automatically executed every 50 steps to clean VRAM fragmentation.

***

### 20.9 VRAM Optimization: Logits Clamp to Prevent Overflow

**Modification Location**: `src/model/aethermind4.py` → LM Head

**Specific Fix**: Clamp logits before computing softmax/cross_entropy:

```python
logits = h @ self.lm_head.weight.T * self.token_emb_scale
logits = logits.clamp(-1e4, 1e4)  # Prevent softmax overflow
```

Under bf16 mixed precision, logits may produce extreme values, causing softmax to return NaN/Inf. Clamping to [-1e4, 1e4] range still provides sufficient differentiation after softmax.

***

### 20.10 New: GPU VRAM Guardian System

**Problem**: During training, the computer would lag (network issues caused other software's cloud pressure to shift locally), other processes would grab GPU VRAM, causing CUDA OOM crashes.

**Modification Location**:

- New file: `src/utils/gpu_guard.py`
- `src/training/train_v4.py`: Called at startup and on OOM

**Specific Fix**:

**(1) Pre-Training Cleanup** `pre_train_gpu_cleanup(auto_kill=True)`:

- Queries all processes occupying GPU VRAM via `nvidia-smi --query-compute-apps`
- Whitelist protection: system processes (System/svchost/dwm), Windows desktop (explorer), NVIDIA drivers, Python training process itself, terminals (cmd/powershell), IDEs (trae/code/cursor)
- Automatically kills all non-protected processes (e.g., browser hardware acceleration, other CUDA programs, etc.)
- Reports released VRAM after cleanup

**(2) OOM Emergency Response** `emergency_free_gpu_memory(threshold_mb=500)`:

- First executes `gc.collect()` + `torch.cuda.empty_cache()` to clean PyTorch internal cache
- If available VRAM is still below threshold, scans and kills current non-protected GPU-occupying processes
- Returns the released VRAM in MB

**(3) OOM Auto-Recovery** (in `_train_step` and `_step_optim`):
...
try:
    ...
    return None  # Skip this batch, continue training
```

***

### 20.11 New: Crash Auto-Restart

**Problem**: Some CUDA OOMs occurred at the kernel level, where Python's try-except could not catch them (directly triggering `c10::Error` causing process abort), and training would exit completely.

**Modification Location**: `train_v4.cmd`

**Specific Fix**: Added `:restart` loop in CMD script:

```batch
:restart
python src\training\train_v4.py --fresh ... (other parameters)
...
- Normal exit (exit code 0): ends
- Abnormal exit (non-0 exit code): waits 15 seconds for VRAM to naturally release, then auto-restarts
- After restart, `train_v4.py` automatically resumes from the latest checkpoint (unless `--fresh` is specified)
- Each restart re-executes `pre_train_gpu_cleanup()` to clean the GPU

***

### 20.12 CMD Script Encoding Fix

**Problem**: Chinese output in CMD scripts was garbled, and `set` commands with Chinese and special characters were interpreted by CMD as commands, causing errors like `'MERS_OFFLINE' is not an internal or external command`.

**Modification Location**: `train_v4.cmd`, `run_inference.cmd`

**Specific Fix**:

1. Added `chcp 65001 >nul 2>&1` at the beginning of files to switch to UTF-8 code page
2. Changed all `echo` output to pure English, avoiding Chinese encoding issues
3. Added `setlocal`/`endlocal` to isolate environment variables
4. Added `set PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True,max_split_size_mb:128` to reduce VRAM fragmentation
5. Added `set HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1` to prevent network requests

***

### 20.13 Adjustment Summary Table

| # | Category | Modified Files | Impact on Model Size | Status |
|---|---|---|---|---|
| 20.1 | Tokenizer Replacement | `configs/`, `dataset.py` | Embedding enlarged (d/L unchanged) | Completed |
| 20.2 | Bug1 Evaluation | `train_v4.py` | None | Completed |
| 20.3 | Bug2 Loss Weights | `aethermind4_config.py` | None | Completed |
| 20.4 | Bug3 Temperature | `aethermind4.py` | None | Completed |
| 20.5 | Bug4 Pheromone | `aethermind4_config.py` | None | Completed |
| 20.6 | VRAM: Checkpointing | `configs/`, `aethermind4.py` | None (saves ~1GB) | Completed |
| 20.7 | VRAM: Skip Softmax | `aethermind4.py` | None (saves ~600MB) | Completed |
| 20.8 | VRAM: Release Tensors | `train_v4.py` | None (saves ~200MB) | Completed |
| 20.9 | VRAM: Logits Clamp | `aethermind4.py` | None | Completed |
| 20.10 | GPU Guardian System | `gpu_guard.py` (new), `train_v4.py` | None | Completed |
| 20.11 | Crash Auto-Restart | `train_v4.cmd` | None | Completed |
| 20.12 | CMD Encoding Fix | `train_v4.cmd`, `run_inference.cmd` | None | Completed |

**Training Verification Results** (step 12000, Phase B):

- Peak VRAM: ~2.79GB / 4.0GB (headroom 1.21GB)
- Loss: 5.59 (decreased from initial 12.14)
- Temperature T: 0.705 (learnable temperature changing normally)
- Pheromone tau: 73.1 (effective deposition)
- bad=0 (no NaN/Inf/OOM-caused bad batches)